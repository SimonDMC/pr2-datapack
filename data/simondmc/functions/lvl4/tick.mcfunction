...
execute as @a[tag=sdmc.in-plot,nbt={Inventory:[{id:"minecraft:axolotl_bucket"}]}] unless entity @s[nbt={Inventory:[{id:"minecraft:axolotl_bucket",tag:{CanPlaceOn:["minecraft:big_dripleaf"]}}]}] run function simondmc:lvl4/replace_axolotl_bucket

# Replace empty bucket
execute as @a[tag=sdmc.in-plot,nbt={Inventory:[{id:"minecraft:bucket"}]}] unless entity @s[nbt={Inventory:[{id:"minecraft:bucket",tag:{CanPlaceOn:["minecraft:water"]}}]}] run function simondmc:lvl4/replace_empty_bucket

# Spawn new axolotl after a second if there are none
execute positioned ~12 ~29 ~16 unless entity @e[type=axolotl,dx=5,dy=4,dz=6,tag=sdmc.in-plot,tag=sdmc.pond-axolotl] run scoreboard players add sdmc.axolotl sdmc.sys 1
