...
# Pick up arrow
execute at @e[tag=pr.target,tag=sdmc.display-arrow] as @a[distance=..1.3,tag=pr.target,nbt=!{Inventory:[{id:"minecraft:arrow"}]},nbt=!{Inventory:[{id:"minecraft:crossbow",tag:{Charged:1b}}]}] run give @s arrow

# Clear arrow if player has charged crossbow
execute as @a[tag=pr.target,nbt={Inventory:[{id:"minecraft:crossbow",tag:{Charged:1b}}]}] run clear @s arrow
